...
export function useInterviewSession(sessionId: string) {
  ...
  const setup = getMockSessionSetup(sessionId);
  ...
  useEffect(() => {
    ...
    const boot = async () => {
      ...
      setPhase("speaking");
      const question = await mockInterviewService.getOpeningQuestion(sessionId);
      setCurrentQuestion(question);
      setMessages(getMockSessionMessages(sessionId));
    };
  ...
  const submitAnswer = useCallback(async () => {
    ...
    try {
      const turn = await mockInterviewService.sendMessage(sessionId, {
        text: answerText,
      });
      ...
      if (turn.isComplete) {
        setPhase("ended");
        const report = await mockInterviewService.end(sessionId);
